...
export const OwnerForm: React.FC<SizeFormProps> = ({ initialData }) => {
  ...
  const onSubmit = async (data: SizeFormValues) => {
    try {
      ...
      if (initialData) {
        await axios.patch(
          `/api/${params.storeId}/owners/${params.ownerId}`,
          data
        );
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.storeId}/owners/${params.ownerId}`);
      router.refresh();
      router.push(`/${params.storeId}/owner`);
      toast.success("Owner deleted.");
    } catch (error: any) {
      toast.error("Make sure you removed all products using this Owner first.");
    } finally {
      setLoading(false);
      setOpen(false);
    }
